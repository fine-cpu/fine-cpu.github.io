...
// 根据当前小时判断是否为深色时间段（18:00 ~ 次日 6:00 为深色）
function isDarkByTime() {
    const hour = new Date().getHours();
    return hour >= 18 || hour < 6;
}

// 解析 !autoSwitch 关键字对应的实际配色方案
function resolveAutoSwitchColor() {
    const isDark = isDarkByTime();
    console.log("%c[I]%c " + `!autoSwitch 自动切换，当前时间 ${new Date().getHours()}:00，深色时段: ${isDark}`);
    return isDark
        ? config.content.theme.colors.autoSwitch.dark
        : config.content.theme.colors.autoSwitch.light;
}
...
class ThemeManager {
    ...
    // 解析主题
    parse() {
        // 构建主题目录（兼容 file:// 协议，去除文件名）
        const baseUrl = window.location.href.substring(0, window.location.href.lastIndexOf('/'));
        themePath = baseUrl + "/assets/themes/" + config.content.theme.theme;

        // 优先使用内嵌主题数据（兼容 file:// 避免 CORS 阻止 XHR）
        if (window.__FIREFLY_THEME__ && window.__FIREFLY_THEME__[config.content.theme.theme]) {
            metaData = window.__FIREFLY_THEME__[config.content.theme.theme];
            console.log("%c[I]%c 从内嵌数据加载主题: " + metaData.name, "background-color: #00896c;", "");
        } else {
            try {
                const xhr = new XMLHttpRequest();
                xhr.open("GET", themePath + "/theme.json", false);
                xhr.send();
        ...
        metaData.colors.index
            .map(key => {
                ...
                if (targetColor === "!autoSwitch") {
                    targetColor = resolveAutoSwitchColor();
                }
...
function loadThemeSelEff() {
    ...
    // 插入新的选中效果类
    const themeItem = document.getElementById(`theme-item-${localStorage.getItem("theme.color")}`);
    if (themeItem) {
        themeItem.setAttribute("class", "theme-item enable");
    }
}
